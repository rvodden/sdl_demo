...
class TicTacToe {
  public:
    ...
    [[nodiscard]] auto getCellState(const uint8_t& x, const uint8_t& y) const -> std::optional<Player> {
  ...
  private:
    std::array<std::optional<Player>, static_cast<std::size_t>(kCellColumns * kCellRows)> _cells {
      std::nullopt,
      std::nullopt,
      std::nullopt,
      std::nullopt,
...
class TicTacToeUI {
      ...
      _eventDispatcher(std::move(dispatcher))
    {
      ...
      for( uint8_t x : std::ranges::iota_view<uint8_t, uint8_t>{ 0, kCellColumns } ) {
        for( uint8_t y : std::ranges::iota_view<uint8_t, uint8_t>{ 0, kCellRows } ) {
          ...
          std::cout << "Creating button at grid (" << static_cast<int>(x) << "," << static_cast<int>(y) << ") -> screen (" << buttonX << "," << buttonY << ") size " << static_cast<uint16_t>(kCellWidth) << "x" << static_cast<uint16_t>(kCellHeight) << "\n";
          auto button = std::make_unique<Button>(_eventDispatcher, FloatRectangle{ buttonX, buttonY, kCellWidth, kCellHeight} );
          button->registerEventHandler([this, x, y, buttonX, buttonY](const MouseButtonEvent& mouseButtonEvent) -> void {
            std::cout << "Button Event at grid (" << static_cast<int>(x) << "," << static_cast<int>(y) << ") button area (" << buttonX << "," << buttonY << "," << static_cast<uint16_t>(kCellWidth) << "," << static_cast<uint16_t>(kCellHeight) << ")!\n";
            auto now = std::chrono::duration_cast<std::chrono::milliseconds>(
                std::chrono::steady_clock::now().time_since_epoch()
            );
            if(mouseButtonEvent.down) { 
              std::cout << "Producing ClickEvent at grid (" << static_cast<uint32_t>(x) << "," << static_cast<uint32_t>(y) << ")\n";
              _eventProducer->produce(std::make_unique<ClickEvent>(now, 0, x, y)); 
              std::cout << "ClickEvent produced for grid (" << static_cast<uint32_t>(x) << "," << static_cast<uint32_t>(y) << ")\n";
            }
          });
    ...
    void render(const std::shared_ptr<TicTacToe>& ticTacToe) {
      ...
      for(uint8_t x : std::ranges::iota_view<uint8_t, uint8_t>{0, kCellColumns}) {
        for(uint8_t y : std::ranges::iota_view<uint8_t, uint8_t>{0, kCellRows}) {
          ...
          _spriteRenderer->render(cellState.value() == TicTacToe::Player::O ? _letterO : _letterX, static_cast<float>(x) * kCellWidth, static_cast<float>(y) * kCellHeight);
        }
      }
      _renderer->present();
    }
  ...
  private:
    ...
    std::shared_ptr<SpriteRenderer> _spriteRenderer = std::make_shared<SpriteRenderer>(_renderer );
    
    std::shared_ptr<Texture> _spriteSheet = std::make_shared<Texture>( *_renderer, &_binary_tic_tac_toe_png_start, ticTacToeSize() );
...
auto main() -> int {
  try {
    ...
    // Debug: Add global mouse event handler to see if any mouse events are received
    eventDispatcher->registerEventHandler<MouseButtonEvent>([](const MouseButtonEvent& mouseEvent) -> void {
      std::cout << "Mouse event received: (" << mouseEvent.x << "," << mouseEvent.y << ") button=" << static_cast<int>(mouseEvent.button) << " down=" << mouseEvent.down << "\n";
    });

    eventDispatcher->registerEventHandler<ClickEvent>([=]([[maybe_unused]] const ClickEvent& clickEvent) -> void {
      std::cout << "Render!\n";
      ticTacToeUI->render(ticTacToe);
    });

    eventDispatcher->registerEventHandler<ClickEvent>([=](const ClickEvent& clickEvent) -> void {
      std::cout << "ClickEvent received! Playing at grid (" << static_cast<uint32_t>(clickEvent.x) << "," << static_cast<uint32_t>(clickEvent.y) << ")\n";
