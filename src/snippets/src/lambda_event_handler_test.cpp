...
public:
    TestEvent(std::chrono::duration<int64_t, std::milli> ts, uint32_t winId, int val)
...
auto main() -> int {
    ...
    eventDispatcher->registerEventHandler<TestEvent>([&counter](const TestEvent& e) -> void {
        counter += e.value;
        std::cout << "Lambda handler 1: Received value " << e.value << ", counter now: " << counter << "\n";
    });
    
    // Test 2: Lambda with different capture
    eventDispatcher->registerEventHandler<TestEvent>([&message](const TestEvent& e) -> void {
    ...
    eventDispatcher->registerEventHandler<TestEvent>([](const TestEvent& e) -> void {
