...
int main() {
    try {
        ...
        auto button = std::make_unique<Button>(eventDispatcher, FloatRectangle{50.0F, 50.0F, 100.0F, 50.0F});

        std::cout << "Created button at (50, 50) with size 100x50\n";
        std::cout << "Button should respond to clicks between x=50-150, y=50-100\n";

        // Register button event handler
        button->registerEventHandler([](const MouseButtonEvent& mouseEvent) -> void {
            std::cout << "BUTTON CLICKED! Mouse at (" << mouseEvent.x << "," << mouseEvent.y << ")\n";
        });
        ...
        eventDispatcher->registerEventHandler<MouseButtonEvent>([](const MouseButtonEvent& mouseEvent) -> void {
            std::cout << "Mouse event: (" << mouseEvent.x << "," << mouseEvent.y << ") button=" 
                      << static_cast<int>(mouseEvent.button) << " down=" << mouseEvent.down << "\n";
        });
