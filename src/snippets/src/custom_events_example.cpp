...
class ScoreUpdateEvent : public CustomUserEvent<ScoreUpdateEvent> {
  public:
    ScoreUpdateEvent(
        ...
        uint32_t winId,
        int32_t playerIdParam,
        int32_t newScoreParam
    ) : CustomUserEvent(ts, winId), 
        playerId(playerIdParam), newScore(newScoreParam) {}
...
class LevelCompleteEvent : public CustomUserEvent<LevelCompleteEvent> {
  public:
    LevelCompleteEvent(
        ...
        level(levelParam), completionTime(completionTimeParam) {}
 ...
 */
class ScoreUpdateHandler : public EventHandler<ScoreUpdateEvent>, public BaseEventHandler {
  public:
    void handle(const ScoreUpdateEvent& event) override {
        std::cout << "Score Update - Player " << event.playerId 
                  << " scored " << event.newScore << " points!" << std::endl;
    }
};
 ...
 */
class LevelCompleteHandler : public EventHandler<LevelCompleteEvent>, public BaseEventHandler {
  public:
    void handle(const LevelCompleteEvent& event) override {
        std::cout << "Level Complete - Level " << event.level 
                  << " completed in " << event.completionTime << " seconds!" << std::endl;
    }
};
...
int main() {
    ...
    std::cout << "  ScoreUpdateEvent: " << ScoreUpdateEvent::getEventType() << std::endl;
    std::cout << "  LevelCompleteEvent: " << LevelCompleteEvent::getEventType() << std::endl;
    std::cout << std::endl;
    ...
    try {
        ...
        std::cout << "Starting event loop (will process a few events then quit)..." << std::endl;
