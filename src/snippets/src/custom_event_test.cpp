...
auto main() -> int {
    try {
        ...
        try {
            ...
            std::cout << "Got event from producer, now checking if handler was called...\n";
            
            // The event should have been dispatched automatically if our fix works
            // Let's manually dispatch it to see what happens
            std::cout << "Manually dispatching event to handler...\n";
            event->handle(*handlerPtr);
            
            std::cout << "Handler called: " << (handlerPtr->handlerCalled ? "YES" : "NO") << "\n";
