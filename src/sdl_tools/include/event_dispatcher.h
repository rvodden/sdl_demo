 ...
 * event routing system. It receives events from an EventProducer and
 * dispatches them to all registered event handlers using the visitor pattern.
 */
 ...
 *
 * The EventDispatcher acts as a mediator between event producers and event
 * handlers. It maintains a collection of registered event handlers and
 * dispatches incoming events to all handlers that can process them. The
 * dispatcher uses the visitor pattern to achieve type-safe event handling
...
class EventDispatcher {
 public:
   ...
   *
   * @param eventProducer Reference to the event producer that will supply
   * events
   */
   ...
   * @endcode
   *
   * @note The dispatcher takes ownership of the created handler and manages
   * its lifetime.
   */
