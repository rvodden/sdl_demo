...
SDLPP_APPLICATION_EXPORT auto SDL_AppInit(void** appstate, [[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) -> SDL_AppResult {
...
SDLPP_APPLICATION_EXPORT auto SDL_AppIterate(void* appstate) -> SDL_AppResult {
  auto* runner = static_cast<sdlpp::ApplicationRunner*>(appstate);
  if (runner == nullptr) {
...
SDLPP_APPLICATION_EXPORT auto SDL_AppQuit(void* appstate, [[maybe_unused]] SDL_AppResult result) -> void {
  auto* runner = static_cast<sdlpp::ApplicationRunner*>(appstate);
  if (runner == nullptr) {
    return;
  }
  ...
  auto& instance = sdlpp::ApplicationRunner::getInstance();
  instance.reset();
}

SDLPP_APPLICATION_EXPORT auto SDL_AppEvent([[maybe_unused]] void* appstate, SDL_Event* event) -> SDL_AppResult {
  if (event == nullptr) {
    return SDL_APP_CONTINUE;
  }
  ...
  if (auto eventBus = instance.getEventBus()) {
    ...
    if (auto sdlEventBus = std::dynamic_pointer_cast<sdlpp::SDLEventBus>(eventBus)) {
