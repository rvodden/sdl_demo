...
Texture::Texture(const Renderer& renderer, const std::filesystem::path& filePath) : _textureImpl { std::make_unique<TextureImpl>() } {
...
Texture::Texture(const Renderer &renderer, const void *location, std::size_t size) : _textureImpl { std::make_unique<TextureImpl>() } {
  ...
  _textureImpl->_sdlTexture = IMG_LoadTexture_IO(renderer._rendererImpl->_sdlRenderer, ioStream, true);
  if  (_textureImpl->_sdlTexture == nullptr ) { throw Exception("IMG_LoadTexture"); }
}
...
void Texture::setTextureBlendMode(const BlendMode &blendMode) {
  bool returnValue = SDL_SetTextureBlendMode(_textureImpl->_sdlTexture, sdlBlendModeMap[blendMode]);
  if( !returnValue ) { throw Exception("SDL_SetTextureBlendMode"); }
}
