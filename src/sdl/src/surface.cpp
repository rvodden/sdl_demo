...
Surface::Surface(const std::filesystem::path& filePath)
  ...
  SDL_Surface* converted = SDL_ConvertSurface(_surfaceImpl->_sdlSurface, SDL_PIXELFORMAT_RGBA8888);
  ...
  SDL_DestroySurface(_surfaceImpl->_sdlSurface);
  _surfaceImpl->_sdlSurface = converted;
}

Surface::Surface(uint32_t width, uint32_t height)
    : _surfaceImpl{std::make_unique<SurfaceImpl>()} {
  _surfaceImpl->_sdlSurface = SDL_CreateSurface(static_cast<int>(width), static_cast<int>(height), SDL_PIXELFORMAT_RGBA8888);
  if (_surfaceImpl->_sdlSurface == nullptr) {
...
Surface::Surface(uint32_t width, uint32_t height, const std::vector<uint8_t>& pixels)
  ...
  // Copy pixel data
  if (_surfaceImpl->_sdlSurface->pixels != nullptr) {
    std::memcpy(_surfaceImpl->_sdlSurface->pixels, pixels.data(), pixels.size());
  } else {
    SDL_DestroySurface(_surfaceImpl->_sdlSurface);
...
auto Surface::getWidth() const -> uint32_t {
  return static_cast<uint32_t>(_surfaceImpl->_sdlSurface->w);
}

auto Surface::getHeight() const -> uint32_t {
  return static_cast<uint32_t>(_surfaceImpl->_sdlSurface->h);
}

auto Surface::getPixels() const -> std::vector<uint8_t> {
  auto* surface = _surfaceImpl->_sdlSurface;
...
void Surface::blit(const Surface& source, uint32_t destX, uint32_t destY) {
  ...
  destRect.w = source._surfaceImpl->_sdlSurface->w;
  destRect.h = source._surfaceImpl->_sdlSurface->h;

  if (!SDL_BlitSurface(source._surfaceImpl->_sdlSurface, nullptr, 
                       _surfaceImpl->_sdlSurface, &destRect)) {
...
void Surface::blit(const Surface& source, uint32_t srcX, uint32_t srcY, 
  ...
  if (!SDL_BlitSurface(source._surfaceImpl->_sdlSurface, &srcRect, 
                       _surfaceImpl->_sdlSurface, &destRect)) {
    throw Exception("SDL_BlitSurface");
  }
}
...
void Surface::save(const std::filesystem::path& filePath) const {
  ...
  if (extension == ".bmp" || extension == ".BMP") {
    if (!SDL_SaveBMP(_surfaceImpl->_sdlSurface, filePathStr.c_str())) {
      throw Exception("SDL_SaveBMP failed: " + filePathStr);
    }
  } else if (extension == ".png" || extension == ".PNG") {
    if (!IMG_SavePNG(_surfaceImpl->_sdlSurface, filePathStr.c_str())) {
      throw Exception("IMG_SavePNG failed: " + filePathStr);
    }
  } else if (extension == ".jpg" || extension == ".jpeg" || extension == ".JPG" || extension == ".JPEG") {
    if (!IMG_SaveJPG(_surfaceImpl->_sdlSurface, filePathStr.c_str(), 90)) {
